...
## One sample "JvRad_Rhipp_204" was identified as an outlier by the PCA plot. Hence it has to be filtered out from the anlysis. The anslysis it rerun, filtering out this sample.
...
# B. Remove the columns with row name JvRad_Rhipp_204 from metadata
...
## Add sample names to PCA plot to identify outlier
```{r}
# Load necessary libraries
library("ggplot2")
library("DESeq2")
...
write.csv(kegg_enrich_df, "KEGG_Enrichment_Results.csv", row.names = FALSE)

# Display the first few rows of the enrichment results
head(kegg_enrich_df)
kegg_enrich_df[0:100, ]
...
write.csv(go_enrich_df, "GO_Biological_Processes_Results.csv", row.names = FALSE)
...
# Save the results as a CSV file
go_enrich_df <- as.data.frame(go_enrich)
write.csv(go_enrich_df, "GO_Molecular_Functions_Results.csv", row.names = FALSE)
...
# Save the results as a CSV file
go_enrich_df <- as.data.frame(go_enrich)
write.csv(go_enrich_df, "GO_Cellular_Components_Results.csv", row.names = FALSE)
